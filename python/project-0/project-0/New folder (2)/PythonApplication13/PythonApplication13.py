...
def main():
    ...
    # ترتیب نتایج بر اساس میزان ترافیک از بیشترین به کمترین
    sorted_traffic_results = dict(sorted(traffic_results.items(), key=lambda item: item[1], reverse=True))

    # تبدیل نتایج به DataFrame
    df = pd.DataFrame(list(sorted_traffic_results.items()), columns=['نقطه', 'ترافیک'])
